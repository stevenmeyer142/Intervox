...
public class PatientData extends DataModel implements Serializable, DataModelListener {
  ...
  CoRegistrations fCoRegistrations = new CoRegistrations(
      fImagesManager.GetFiducialsAccess(SingleSpaceID.GetDefaultVirtualSpaceID()));
   ...
   * };
   */
  private transient ImageSettingsFactory fImageSettingsFactory;
  ...
  private static int fgFileCounter = 0;
  static final int kVersionNum = 1;
  static final long serialVersionUID = -8636249877080612702L;
  public static final boolean kDontSerializePatientData = true;
   ...
   * PostChanged(new DataChangedEvent(this, DataChangedEvent.CHANGED, 0,
   * kNewLayoutString));
   * }
   */
  ...
  public void WriteFile(File file) throws IOException {
     ...
     * fConfigurationManager.GetSelectedConfiguration();
     * 
  ...
  public static PatientData ReadFile(
      File file, ProgressBroadcaster broadcaster, TrackerSettingsAccess trackerSettings)
      throws IOException {
    ...
    try {
      ...
      if (kDontSerializePatientData) {
        ...
        PatientDataSeriesFileLoaderThread thread = new PatientDataSeriesFileLoaderThread(result, broadcaster, objectIn,
            trackerSettings);
   ...
   * {
   * PostChanged(new DataChangedEvent(this, DataChangedEvent.START_L0ADING,
   * loadingNum, message));
   * }
   */
  /*
   ...
   * {
   * PostChanged(new DataChangedEvent(this, DataChangedEvent.L0ADING, value,
   * message));
   * }
   */
  ...
  protected void SetupCoRegistrations() {
    fCoRegistrations.SetFiducialsAccess(
        fImagesManager.GetFiducialsAccess(SingleSpaceID.GetDefaultVirtualSpaceID()));
   ...
   * {
   * PostChanged(new DataChangedEvent(this, DataChangedEvent.START_L0ADING, high,
   * message));
   * }
   * 
...
class PatientDataSeriesFileLoaderThread extends Thread {
  ...
  public PatientDataSeriesFileLoaderThread(
      PatientData data,
      ProgressBroadcaster broadcaster,
      ObjectInput input,
      TrackerSettingsAccess settings) {
    fData = data;
