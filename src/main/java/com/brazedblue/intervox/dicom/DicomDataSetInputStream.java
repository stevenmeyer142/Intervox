...
public class DicomDataSetInputStream extends InputStream {
  ...
  private static final String kDicomLabelString = "DICM";
  private static final int kInitialIgnoreBytes = 128;

  public static final int kFloatDoubleSize = 8;
  public static final int kFloatSingleSize = 4;
  public static final int kOtherWordSize = 2;
  public static final int kLongSize = 4;
  public static final int kShortSize = 2;
  ...
  public static final boolean kOpenEarlyFormatDicom = true;
    ...
    if (fBufferSize > fBuffer.length) {
      ...
      if (Debug.fgDebugging) {
        NeuroSynchUtil.ErrorMessage(
            "DicomDataSetInputStream's buffer needed to be resized.", NeuroSynchUtil.kNoDlog);
      }
      fBuffer = new byte[fBufferSize];
    }
  ...
  public DicomValueRep ReadValueRep() throws IOException {
    ...
    // I don't think this is exactly right, the app should know if implicit or
    // explicit value
    // reps are used by the data element. The default is "Implicit VR Little Endian
    // Transfer
    // Syntax"
    // but the files I have been reading are mixed explicit and implicit
    // If the length value randomly generates a true result for the if expression,
    // the following
  ...
  public long ReadLittleEndianLong(int numOfBytes) throws IOException {
    ...
    if (numOfBytes > 8) {
      NeuroSynchUtil.ErrorMessage(
          "range error in DicomDataSetInputStream.ReadLittlEndianLong", NeuroSynchUtil.kNoDlog);
    }
