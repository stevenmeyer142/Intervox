...
public class DicomFileLoaderThread extends FileLoaderThread /* implements anInterface */ {
  ...
  private DicomDataSetInputStream fInputStream = new DicomDataSetInputStream(200000); // files are smaller than 2k
  ...
  public void run() {
    try {
      ...
      for (int i = 0; i < fFileList.length; i++) {
        ...
        DicomImageDataSet dataSet = ImageHandler.GetDicomDataSet(fDirectory + fFileList[i], fInputStream);
        if (dataSet != null) {
      ...
      if (fReceiver.SeriesAddedCount() > 1) {
        ...
        String result = NeuroSynchUtil.AskUser(
            "Save Largest Series",
