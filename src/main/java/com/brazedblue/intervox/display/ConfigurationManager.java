...
public class ConfigurationManager {
  ...
  private static final String kDefaultConfigurationName = "Original" + kDefaultConfigurationCode + kConfigurationSuffix;
  ...
  public Configuration GetConfigurationFromUser() {
    ...
    if (configurations.length > 1) {
      String fileName = NeuroSynchUtil.GetUserInput(
   ...
   * catch (IOException e)
   * {
   * NeuroSynchUtil.ErrorMessage("ConfigurationManager.SetDefaultConfiguration- "
   * + e.getMessage(), NeuroSynchUtil.kNoDlog);
   * }
   * 
  ...
  public void SetDefaultConfiguration(Configuration config) {
    ...
    if (!currentFileName.equals(newFileName)) {
      ...
      newFileName = newFileName.substring(0, newFileName.length() - kConfigurationSuffix.length())
  ...
  void AddConfigurationFile(Frame frame) {
    // File newFile = FileHandler.GetFile(frame, "Add File in this Directory",
    // GetFileFilter());
  ...
  public void EditConfigurations(
    ...
    Configuration selectedConfig = (selectedData != null) ? selectedData.GetConfiguration() : GetDefaultConfiguration();
