...
public class ImageSeriesView extends JPanel implements NSImageView {
  ...
  private static final String kAddFiducialStr = "Add Point";
  private static final String kDeleteFiducialStr = "Delete Point";

  private static final String kRightLabelString = "RIGHT";
  private static final String kLeftLabelString = "LEFT";

  private static final boolean kDebugDrawing = false;
  ...
  /*
   * public void DisplayTrackerOutput(DeviceOutput output, TransformerAccess
   * transformer)
   * {
  ...
  // move this all out into it's own panel, scrollbars in seperate panel, labels,
  // controls also
  protected Dimension GetImageViewSize() {
    ...
    {
      result.setSize(
          fImageViewType.GetMagnification() * 100, fImageViewType.GetMagnification() * 100);
    }
  ...
  public ImageControlSettings GetImageControlSettings() {
    if (fImageControlSettings == null) {
      fImageControlSettings = IsEditingConfiguration()
          ? DefaultImageControlSettings.GetEditConfigSettings()
          : fImageSettingsFactory.GetCurrentSettings();
    }
  ...
  private void SetTrajectorySlice(DeviceOutput output, TransformerAccess transformer) {
    ...
    if (rotation != null && realPoint != null) {
      ...
      fPerspective.SetTrajectorySlice(
          transformer, output.GetSpaceID(), new FloatPoint(realPoint), xIncVector, yIncVector);
    }
  }

  public Dimension getPreferredSize() {
    int labelHeight = (fDescription != null && fDescription.isVisible())
        ? fDescription.getPreferredSize().height
        : 0;
  ...
  public void doLayout() {
    ...
    if (scrollbarWidth > 0) {
      fScrollbar.setBounds(
          preferredSize.width - scrollbarWidth, 0, scrollbarWidth, imageSize.height);
    }
   ...
   * public ImageControlSettings GetImageControlSettings()
   * {
   * return (fImageSeriesViewControls != null) ?
   * fImageSeriesViewControls.GetImageControlSettings() : null;
   * }
   */
  ...
  FiducialPoint GetFiducialForPoint(Point pt) {
    ...
    for (int i = 0; i < fiducials.length; i++) {
      ...
      if (slice == GetCurrentImageNumber()) {
        ...
        if (NeuroSynchUtil.GetDistanceBetweenPts(fiducialPt.x, fiducialPt.y, pt.x,
            pt.y) <= (kFiducialCircleSize + 2 * kFiducialLineWidth)) {
          result = fiducials[i];
          break;
        }
      }
      ...
      if (fPoints != null) {
        ...
        for (int i = 0; i < fiducials.length; i++) {
          ...
          if (slice == currentImage) {
            ...
            if (fDiplayingRegistrations
                && fRegistrations != null
                && fRegistrations.HasLocation(location, null)) {
              color = Color.blue;
            }
            ...
            for (int j = 0; j < kFiducialLineWidth; j++) {
              g.drawOval(
                  drawPt.x - kFiducialCircleSize - j,
                  drawPt.y - kFiducialCircleSize - j,
                  2 * (kFiducialCircleSize + j) + 1,
                  2 * (kFiducialCircleSize + j) + 1);
            }
          }
        }
  ...
  class ImageSeriesViewLeftRightLabelAdorner implements ComponentAdorner {
    ...
    public void Paint(Graphics g, AdornedComponent c) {
      ...
      if (axis == OrthoImageSetController.kCoronal || axis == OrthoImageSetController.kAxial) {
        ...
        g.drawString(
            kLeftLabelString,
            imageSize.width - kInset - fMetrics.stringWidth(kLeftLabelString),
  ...
  class ImageSeriesViewFocalPointAdorner implements ComponentAdorner {
    ...
    private void SetFocalPointLocation(Location3D focus) {
      ...
      if (Debug.fgDebugging
          && false
          && fImageViewType.HasFeature(fImageViewType.kTracksInterceptFeature)) {
    ...
    public void Paint(Graphics g, AdornedComponent c) {
      if (fImageAdorner.GetImage() != null) {
        if (Debug.fgDebugging && kDebugDrawing) {
          System.out.println(
              "Paint focus " + fFocus + " slice " + Integer.toString(GetCurrentImageNumber()));
        }
  ...
  class ImageSeriesViewFocusHandler extends MouseAdapter
      implements FocalPointListener, AdjustmentListener {
  ...
  private class ImageSeriesViewInterceptDistanceText implements ComponentAdorner {
    ...
    /* static */ private final String kMaxDistanceText = "999.9";
    // ~10 meters)
  ...
  private class ImageSeriesViewInterceptFocalPoint extends ImageSeriesViewFocusHandler {
     ...
     * if (Debug.fgDebugging)
     * {
     * System.out.println("ImageSeriesViewInterceptFocalPoint.SetScrollbar  " +
     * point.toString());
     * }
     * }
     */
    ...
    protected void UpdateFocalPoint(Location3D focalPoint) {
      if (fScrollbar != null) {
        ...
        if (Debug.fgDebugging && false) {
          System.out.println(
              "ImageSeriesViewInterceptFocalPoint.UpdateFocalPoint  "
                  + nonScrolledLocation.toString());
        }
    ...
    public void DisplayTrackerOutput(DeviceOutput output, TransformerAccess transformer) {
      ...
      if (transformer.CanTransform(output.GetSpaceID(), GetSpaceID())
        ...
        if (distance >= 0) {
          ...
          inverseTransformedPoint = transformer.TransformPoint(
              inverseTransformedPoint, GetSpaceID(), output.GetSpaceID());
        ...
        if (Debug.fgDebugging && false) {
          System.out.println(
              "ImageSeriesViewInterceptFocalPoint.DisplayTrackerOutput  " + floatPoint.toString());
        }
  ...
  /*
   * scroll up and down one increment with 'u' and 'd' in case the scrollbars do
   * not have arrows (with Mac for instance)
   */
