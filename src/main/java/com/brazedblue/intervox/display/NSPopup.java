...
public class NSPopup implements MouseListener, MouseMotionListener, ActionListener {
  ...
  private static DrawRubber3D f3DDraw = new DrawRubber3D();
  private static final int kInset = 3;

  private static final int kDefaultWidth = 40;
  private static final int kDefaultItemHt = 10;

  private static final int kNoSelection = -1;
  ...
  private Rectangle GetItemRectangle(int item) {
    ...
    return new Rectangle(
        fLocation.x + kInset,
        fLocation.y + kInset + item * itemHeight,
        fLocation.width - 2 * kInset,
        itemHeight);
  }
