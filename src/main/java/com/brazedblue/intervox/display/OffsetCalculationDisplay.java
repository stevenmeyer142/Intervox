...
public class OffsetCalculationDisplay extends JPanel implements ActionListener {
  ...
  private static final String kRecordPosition = "Record Position";
  private static final String kCalcOffsetPosition = "Calculate Offset";
  private static final String kAddOffset = "Add";
  private static final String kDeleteOffset = "Delete";
  private static final String kRenameOffset = "Rename";

  private static final String kGenericOffsetString = "Offset";

  private static final int kMinimumPoints = 4;

  private static boolean kDebugDump = Debug.fgDebugging && false;
  private static FloatPoint gOffsetPoint = new FloatPoint();

  private static final boolean kMessageIfNULL = true;
  ...
  public static synchronized void OffsetPoint(
      FloatPoint transformedPt, FloatPoint offset, Matrix3 rotation) {
   ...
   * Pt(center) = [X(c), Y(c), Z(c)] Center of sphere where points lie
   * aproximately on the surface
   * R = radius of sphere
   *
   *
   ...
   * sqr(X(j) - X(c)) + sqr(Y(j) - Y(c)) + sqr(Z(j) - Z(c)) = sqr(X(1) - X(c)) +
   * sqr(Y(1) - Y(c)) + sqr(Z(1) - Z(c))
   *
   * expanding the squares
   * sqr(X(j)) - 2 * X(j) * X(c) + sqr(X(c)) + sqr(Y(j)) - 2 * Y(j) * Y(c) +
   * sqr(Y(c)) + sqr(Z(j)) - 2 * Z(j) * Z(c) + sqr(Z(c))
   * = sqr(X(1)) - 2 * X(1) * X(c) + sqr(X(c)) + sqr(Y(1)) - 2 * Y(1) * Y(c) +
   * sqr(Y(c)) + sqr(Z(1)) - 2 * Z(1) * Z(c) + sqr(Z(c))
   *
   * Solving for X(c), Y(c), Z(c)
   *
   * 2 * X(1) * X(c) - 2 * X(j) * X(c) + 2 * Y(1) * Y(c) - 2 * Y(j) * Y(c) + 2 *
   * Z(1) * Z(c) - 2 * Z(j) * Z(c)
   ...
   * | Z(c) |
   *
   * b = |(sqr(X(1)) - sqr(X(2)) + sqr(Y(1)) - sqr(Y(2)) + sqr(Z(1)) - sqr(Z(2)))
   * / 2 |
   * | ... |
   * | ... |
   * |(sqr(X(1)) - sqr(X(j)) + sqr(Y(1)) - sqr(Y(j)) + sqr(Z(1)) - sqr(Z(j))) / 2
  ...
  private void CalculateOffsetPosition1() {
    ...
    if (offset != null && offset.GetPointCount() >= kMinimumPoints) {
      ...
      // I use Matrix3 because I haven't done an inverse implementation for
      // ArbitraryFloatMatrix
      Matrix3 tempMatrix3 = new Matrix3();
      tempMatrix3.Set(invertMatrix);
      tempMatrix3.invert();
      invertMatrix.Set(tempMatrix3);

      // calculation will set matrixB to resulting [3 X 1] matrix with rows equaling
      // x,y,z
  ...
  class OffsetCalculationTracker implements TrackerListener, ActionListener {
    ...
    OffsetCalculationTracker(TrackerController controller, AbstractButton regButton) {
      ...
      controller.AddTrackerListener(
          this, 0, true, DeviceOutput.ALL_STATIONS); // 3rd arg wants transformation
        ...
        if (output.GetStation() == DeviceOutput.STATION1) {
          ...
          if (rotation != null) {
            ...
            NeuroSynchUtil.ErrorMessage(
                "OffsetCalculationTracker.DataRetrieved: No angle orientation for point",
