...
public class RegistrationCntrlsDisplay extends ControlsDisplay {
  ...
  public RegistrationCntrlsDisplay(TrackerController tracker, OrthoImageSetController controller) {
    ...
    fFiducialControls2 = new FiducialControlsDisplay(
        (tracker != null)
            ? SingleSpaceID.GetRealSpaceID()
            : SingleSpaceID.GetDefaultVirtualSpaceID(),
        fShowsLocalizerSpace,
        tracker);
  ...
  private RegistrationsAccess GetRegistrationsAccess() {
    return fCoRegistrations.GetRegistrationsAccess(
        fFiducialControls1.GetSpaceID(), fFiducialControls2.GetSpaceID());
  }
  ...
  private void UnlinkSpaces() {
    // fCoRegistrations.GetTransformerAccess().InstallTransformation(null,
    // fFiducialControls1.GetSpaceID(), fFiducialControls2.GetSpaceID());
  }
  ...
  /*
   * class RegistrationTrackerPt implements TrackerListener, KeyListener,
   * ActionListener
   * {
   ...
   * fTracker = controller;
   * 
   * controller.AddTrackerListener(this, 0, true, DeviceOutput.ALL_STATIONS); //
   * 3rd arg wants transformation
   ...
   * {
   * DeviceOutput ref = ((fLastReference != null) && fLastReference.IsFresh()) ?
   * fLastReference : null;
   * 
   * registrations.RegisterPair(new LocalizerFiducialPoint(fLastPoint, ref),
   * selectedPt);
   * java.awt.Toolkit.getDefaultToolkit().beep();
   ...
   * else
   * {
   * NeuroSynchUtil.ErrorMessage("Not receiving tracker location",
   ...
   * 
   */
  private class RegistrationCntrlsDisplaySpaceListener
    ...
    private RegistrationCntrlsDisplaySpaceListener(
        ImageSpaceSelector selector1, ImageSpaceSelector selector2) {
    ...
    public void SpaceSelected(ImageSpaceSelector.SelectEvent event) {
      if (!fPauseListening) {
        ...
        try {
          ...
          ImageSpaceSelector otherSelector = (selector == fSelector1) ? fSelector2 : fSelector1;

          otherSelector.ShowSpace(event.GetSpaceID(), !event.GetSelected());

          fCoRegistrations.SetSelectedSpaceLink(
              fSelector1.GetSelectedSpace(), fSelector2.GetSelectedSpace());
          UnlinkSpaces();
        } finally {
