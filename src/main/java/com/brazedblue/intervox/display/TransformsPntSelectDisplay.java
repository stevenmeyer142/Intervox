...
public class TransformsPntSelectDisplay extends TransformsSelectDisplay
  ...
  private static final int kMinPoints = 3;
  ...
  private void SetTransformation(boolean showDialog) {
    ...
    if (numOfSelectedButtons >= kMinPoints) {
      ...
      try {
        TransformerAccess transformer = GetTransformerAccess();
        TransformationMatrix newMatrix = new TransformationMatrix(selectedRegistrations,
            transformer.IsUsingReference());
        ...
        NeuroSynchUtil.ErrorMessage(
            "Couldn't compute transformation: " + e.getMessage(), NeuroSynchUtil.kShowDlog);
      }
    } else if (showDialog) {
  ...
  private void SetPointErrors(TransformationMatrix newMatrix) throws ComputationException {
    RegistrationTransformer regTransformer = new RegistrationTransformer(newMatrix, fToSpace.GetUnit(),
        Unit.MILLIMETER);
    ...
    for (int i = 0; i < fRegistrations.length; i++) {
      ...
      label = label.substring(0, replaceStart)
          + TransformationsDisplay.kAccuracyStartString
          + NeuroSynchUtil.GetDecimalString(distance, 2)
          + TransformationsDisplay.kAccuracyEndString;
   ...
   * {
   * return (fData != null) ? fData.GetCoRegistrations().GetTransformerAccess() :
   * null;
   * }
   * 
