...
public class RegionSelectorFrame extends NSInternalFrame implements ActionListener {
	...
	static private final int	kTitleJustification = TitledBorder.DEFAULT_JUSTIFICATION; 
	static private final int 	kTitlePosition = TitledBorder.TOP;
	
	static private final BevelBorder kBevelBorder = new BevelBorder(BevelBorder.RAISED);

	static private final String kDefaultRegion = null;
	
	static private final String kClearSeedPtsString = "Clear Seed Points";
	static private final String kSliceClearSeedPtsCmdString = kClearSeedPtsString;
	static private final String kAllSlicesClearSeedPtsCmdString = "All " + kClearSeedPtsString;
	
	static private final String kClearEdgeLinesString = "Clear Edge Lines";
	static private final String kSliceClearEdgeLinesCmdString = kClearEdgeLinesString;
	static private final String kAllSlicesClearEdgeLinesCmdString = "All " + kClearEdgeLinesString;

	static private final String kShow3DString = "Generate 3D";
	static private final String kSetColorString = "Set Color";
	
	static private final String kAddRegionVersionString = "Add Region Version";

	static private final String kRegionPrefixString = "Region ";
	
	static public final boolean kTrace = false;
...
/*	public void stateChanged(ChangeEvent e)
	...
	private void CreateAllSlicesPanel(Container parent, int width)
	{
		...
		btnPanel.setBorder(new TitledBorder(kBevelBorder, "All Slices", kTitleJustification, kTitlePosition));
		parent.add(btnPanel);
	...
	private void CreateCurrentSlicePanel(Container parent, int width)
	{
		// add button panel
		JPanel btnPanel = new JPanel();
		btnPanel.setBorder(new TitledBorder(kBevelBorder, "Current Slice", kTitleJustification, kTitlePosition));
		parent.add(btnPanel);
		btnPanel.setLayout(new BoxLayout(btnPanel, BoxLayout.X_AXIS));
		JButton clearButton = new JButton(kClearSeedPtsString);
	...
	private void Create3DSettingsPanel(Container parent, int width)
	{
		// add button panel
		JPanel btnPanel = new JPanel();
		btnPanel.setBorder(new TitledBorder(kBevelBorder, "Region Settings", kTitleJustification, kTitlePosition));
		parent.add(btnPanel);
		...
		labelTable.put( new Integer(fTransparencySlider.getMinimum()), new JLabel( "Transparent", JLabel.LEFT ) );
		labelTable.put( new Integer(fTransparencySlider.getMaximum()), new JLabel( "Opaque", JLabel.RIGHT ) );
...
*/	
	...
	{
		float transparency = (float)fTransparencySlider.getValue() / 
	...
	class RegionAdjustmentListener extends MouseAdapter implements AdjustmentListener, MouseMotionListener
	{
		...
		public void adjustmentValueChanged(AdjustmentEvent e)
		{
			if (kTrace)
			{
				System.out.println("RegionAdjustmentListener.adjustmentValueChanged. event- " + e.toString());
			}
	...
	private class RegionSelectorFrameSpaceChanger implements ImageSpaceSelector.ImageSpaceSelectListener
	{
		ImageViewType		fImageViewType;
		
		
		private RegionSelectorFrameSpaceChanger(ImageViewType imageViewType, ImageSpaceSelector spaceSelector)
		{
	...
	private class RegionSelectorFrameRangeValuesHandler
	{
		...
		private RegionSelectorFrameRangeValuesHandler(ImageFilterAccessor filterAccessor, Histogram histogram)
		{
			fImageFilter = filterAccessor;
			fHistogram = histogram;
			
			SetImageAdjustments(fImageFilter.GetImageAdjustments(SingleSpaceID.GetDefaultVirtualSpaceID()));
		}
